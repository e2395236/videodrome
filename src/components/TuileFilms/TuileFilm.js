...
function TuileFilm({ data, filtreActif }) {
  ...
  // Condition pour afficher le réalisateur ou l'année en fonction du filtre
  let infoSupplementaire;
  if (filtreActif === 'realAsc' || filtreActif === 'realDesc') {
    infoSupplementaire = <p>Réalisation: {data.realisation}</p>;
    console.log(data.realisation);

   
  } else if (filtreActif === 'anneeAsc' || filtreActif === 'anneeDesc') {
    infoSupplementaire = <p>({data.annee})</p>;
